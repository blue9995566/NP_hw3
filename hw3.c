...
int main(void)
{
    ...
    for(int i=0;i<5;i++){
        ...
        // printf("  client%d's port : %d",i+1,clients[i].port);
        // printf("  client%d's filename : %s",i+1,clients[i].filename);
        if(create_connect(i)<0) clients[i].status = F_DONE; //something wrong!
        // printf("  client%d's fd,status : %d,%d",i+1,clients[i].fd,clients[i].status);
    }
    // create_connect(clients[0]);

    int a=0;
    /*for(int i=0;i<5;i++){
        int fd=clients[i].fd;
        int ind=fdtoindex(fd);
        printf("<br>(%d,%d,status:%d) ",fd,ind,clients[ind].status);
    }*/
    
    /*fp = fopen(clients[0].filename, "r"); // filename
    if (fp == NULL)
    {
...
int create_connect(int index){
    // printf("<script>document.all['m0'].innerHTML +=\"");
    // printf("<br>client's ip : %s\n",clients[index].ip);
    // printf("  client's port : %d",clients[index].port);
    // printf("  client's filename : %s",clients[index].filename);
    // printf("\";</script>\n");
    // fflush(stdout);
    
    struct hostent *he;
    if ((he = gethostbyname(clients[index].ip)) == NULL)
	{
	...
	client_sin.sin_port = htons((u_short)clients[index].port);
